...
export class XBTUSD摸顶抄底追涨追跌 implements PositionAndOrderTask {
    ...
    private bitmex_止损(self: PositionAndOrder) {
    ...
    private bitmex_开仓(self: PositionAndOrder) {
        ...
        if (本地维护仓位数量 === 0 && 仓位数量 === 0 && 活动委托.length === 0 && 信号灯Type !== 'none') {
            ...
            if (
                (lastNumber(self.realData.dataExt['XBTUSD'].bitmex.时间) - this.state.最后一次信号时间 < 20 * 1000) && //没有超时
                (                                             //抵消
                    (this.state.最后一次信号 === '追涨' && 信号灯Type === '摸顶') ||
                    (this.state.最后一次信号 === '追跌' && 信号灯Type === '抄底') ||
                    (this.state.最后一次信号 === '抄底' && 信号灯Type === '追跌') ||
                    (this.state.最后一次信号 === '摸顶' && 信号灯Type === '追涨')
                )
            ) {
                return false
            }

            this.state.最后一次信号 = 信号灯Type
            this.state.最后一次信号时间 = lastNumber(self.realData.dataExt['XBTUSD'].bitmex.时间)


            const 市价 = 信号灯Type === '追涨' || 信号灯Type === '追跌' || lastNumber(self.realData.dataExt.XBTUSD.bitmex.绝对价差) > 15


            if (lastNumber(self.realData.dataExt['XBTUSD'].bitmex.时间) > this.state.到什么时间不开仓) {
                return 市价 ?
                    self.taker({
                        symbol: 'XBTUSD',
                        side: 开仓side,
                ...
                if (活动委托.length === 1) {
                    if (活动委托[0].type === '限价只减仓' && 活动委托[0].side === 平仓side) {
                        if (活动委托[0].price !== get位置1价格()) {
                            ...
                            }, this.state.最后一次信号 + '平仓' + '  ' + 亏损挂单平仓Text + '  重新挂' + 平仓side + '1')
                        } else {
                            return false
                        }
                    }
            ...
            if (信号side === 平仓side && 活动委托.length === 0) {
                ...
                return 市价 ?
                    self.taker({
                        ...
                        side: 平仓side,
                        size: Math.abs(仓位数量),
                        text: this.state.最后一次信号 + '平仓' + ' 触发了反向开仓信号  ' + '自动止盈波段step 全部止盈',
                    }, 信号side + ' 信号msg:' + 信号msg) :
                    self.maker({
                        symbol: 'XBTUSD',
                        side: 平仓side,
