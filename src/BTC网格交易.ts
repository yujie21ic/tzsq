...
const get加仓数量 = (累计: number) => 网格配置.加仓.get单个格子数量(累计 + Math.abs(api.myPosition))

const get减仓数量 = (累计: number) => 网格配置.减仓.get单个格子数量(累计 + Math.abs(api.myPosition))

const get开仓均价 = () => Math.round(api.开仓均价 * 网格配置.价钱除以多少)

const getLastFillPrice = () => Math.round(api.lastFillPrice * 网格配置.价钱除以多少)

const getBuy1 = () => Math.round(api.buy1 * 网格配置.价钱除以多少)

const getSell1 = () => Math.round(api.sell1 * 网格配置.价钱除以多少)
...
const sync活动委托 = (arr: { side: BaseType.Side, price: number, count: number }[]) => {
    ...
    api.活动委托.forEach(v => {
        ...
        const PRICE = Math.round(v.price * 网格配置.价钱除以多少)//<--------- 
    ...
    if (cancelIDs.length !== 0) {
        ...
        for (const price in dic) {
            arr.push({
                side: dic[price].side,
                price: Number(price) / 网格配置.价钱除以多少,//<---------
