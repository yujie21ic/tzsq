...
const parse1 = compose(typeObjectParse({ errcode: -1 }), safeJSONParse)

const send = (msg: string, access_token: string) => new Promise((resolve, reject) => {
    request({
        url: 'https://qyapi.weixin.qq.com/cgi-bin/message/send?' + queryStringStringify({
            access_token
        }),
        method: 'POST',
        body: JSON.stringify({
...
const parse2 = compose(typeObjectParse({ access_token: '' }), safeJSONParse)
