...
export class RealDataBase {
    ...
    private item2(xxx: {
        ...
        return {
            ...
            价格乘以ln净成交量: 指标.map(() => Math.min(收盘价.length, 买.净成交量.length), i => 收盘价[i] * 买.净成交量[i]),
            价格乘以ln主动买: 指标.map(() => Math.min(收盘价.length, xxx.吃单情况.length), i => 收盘价[i] * xxx.吃单情况[i].卖.被吃量),
            价格乘以ln主动卖: 指标.map(() => Math.min(收盘价.length, xxx.吃单情况.length), i => 收盘价[i] * xxx.吃单情况[i].买.被吃量),
